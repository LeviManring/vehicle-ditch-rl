...
% dynamics scenario
%
% This code is modified for our particular scenario from 
...
dt = params.dt;                         % [s] sampling time
T = 22;                           % [s] initial prediction horizon time
...
% 3. Plant structure
% dynamics ode function
plant.dynamics = @(t,y,tauA) pilcoRWD_ode(t,y,tauA,params,curves);
...
policy.fcn = @(policy,m,s)conCat(@congp,@gSat,policy,m,s);% controller 
                                                          % representation
policy.maxU = params.dim.maxT;                                         % max. amplitude of 
                                                          % control
...
policy.p.targets = 0.1*randn(nc, length(policy.maxU));    % init. policy targets 
                                                          % (close to zero)
policy.p.hyp = log([1 1 1 1])';              % initialize policy
                                                          % hyper-parameters

% 5. Set up the cost structure
cost.fcn = @loss_curves;                       % cost function
cost.gamma = 1;                            % discount factor
cost.expl =  0.0;                          % exploration parameter (UCB)
cost.angle = plant.angi;                   % index of angle (for cost function)
cost.target = [9.5   0]';                       % target state
cost.width = abs(mu0(1) - cost.target(1))/10;                        % cost function width
...
opt.MFEPLS = 50;                         % max. number of function evaluations
                                         % per line search
