function [states, cond_end, event_num, failed] = curves_dynamicsAWD_nr(init_conds,cond_start,tauA,tauB,t_int,params,curves,refine)
%% curves_dynamicsAWD_nr
...
switch cond_start
    case 1 % noslip
        ...
        % or 4)
        FeventA = frictionterminalcond_curves(y0,tauA,tauB,cond_start,'A',params,curves);
        FeventB = frictionterminalcond_curves(y0,tauA,tauB,cond_start,'B',params,curves);
    ...
    case 3 % rearslipfrontstick
        % check if tauB makes the front slip (switches to case 2)
        FeventB = frictionterminalcond_curves(y0,tauA,tauB,cond_start,'B',params,curves);
        if FeventB < 0
            cond_start = 2;
        end
    case 4 % rearstickfrontslip
        % check to see if tauA makes the rear slip (switches to case 2)
        FeventA = frictionterminalcond_curves(y0,tauA,tauB,cond_start,'A',params,curves);
        if FeventA < 0
            cond_start = 2;
        end
...
while t_init < t_final
    ...
    odestart = @(t,y) curves_selector_ode(t,y,tauA,tauB,cond_start,params,curves);
    ...
    switch cond_start
        case 1 % no slip
            ...
            eventhandle1 = @(y) frictionterminalcond_curves(y,tauA,tauB,cond_start,'A',params,curves);
            eventhandle2 = @(y) frictionterminalcond_curves(y,tauA,tauB,cond_start,'B',params,curves);
            deventhandle1 = @(y) dfrictionterminalcond_curves(y,tauA,tauB,cond_start,'A',params,curves);
            deventhandle2 = @(y) dfrictionterminalcond_curves(y,tauA,tauB,cond_start,'B',params,curves);
            eventerr1 = f_err;
            eventerr2 = v_err;
            caseswitch1 = 3;
            caseswitch2 = 4;
        case 2 % allslip
            ...
            eventhandle1 = @(y) velocityterminalcond_curves(y,'A',params,curves);
            eventhandle2 = @(y) velocityterminalcond_curves(y,'B',params,curves);
            deventhandle1 = @(y) dvelocityterminalcond_curves(y,tauA,tauB,cond_start,'A',params,curves);
            deventhandle2 = @(y) dvelocityterminalcond_curves(y,tauA,tauB,cond_start,'B',params,curves);
            eventerr1 = v_err;
            eventerr2 = v_err;
            caseswitch1 = 4;
            caseswitch2 = 3;
    ...
    [zero_crossing1, guess_ind1] = zerocrossing(term1(:,1),eventerr1);
    [zero_crossing2, guess_ind2] = zerocrossing(term2(:,1),eventerr2);
    ...
    % Solve for the event
    [tevent, yevent, failed, nozero] = nr_eventdetectionAWD(odestart, eventhandle, [t0_nr, tk], y0_nr, max_err, max_nr_steps);
...
v_relB = v_relB_fcn_curves(y(end,:),params,curves);
