function video_fun_AWD(x,thetaM,tauA,tauB,titledescription,params,record,framerate,videoname)
%% video_fun_AWD
...
%   tauA: Nx1 array containing the applied torque to wheel A
%   tauB: Nx1 array containing the applied torque to wheel B
...
if abs(max(tauB)) ~= 0
    tauB_sc = tauB/max(abs(tauB));
else
    tauB_sc = tauB;
end
if abs(max(tauA)) ~= 0
    tauA_sc = tauA/max(abs(tauA));
else
    tauA_sc = tauA;
end
...
lower = subplot(2,1,2);
grid on; hold on;
p_tauA      = plot(0,tauA_sc(1,1),'r-','linewidth',2);
p_tauB        = plot(0,tauB_sc(1,1),'b-','linewidth',2);
p_tauA_inst = plot(0,tauA_sc(1,1),'rx','markersize',16,'linewidth',2);
p_tauB_inst   = plot(0,tauB_sc(1,1),'bo','markersize',16,'linewidth',2);
xlabel('$t$ (s)','Interpreter','Latex'); ylabel('Nondimensional Values','Interpreter','Latex');
legend('$\hat{\tau}_A$','$\hat{\tau}_B$','Interpreter','Latex'); 
...
for k = 2:length(x)
    ...
    p_tauA.XData = [p_tauA.XData, dt*(k-1)];
    p_tauA.YData = [p_tauA.YData, tauA_sc(k,1)];
    
    p_tauB.XData = [p_tauB.XData, dt*(k-1)];
    p_tauB.YData = [p_tauB.YData, tauB_sc(k,1)];
    
    p_tauA_inst.XData = dt*(k-1);
    p_tauA_inst.YData = tauA_sc(k,1);
    
    p_tauB_inst.XData = dt*(k-1);
    p_tauB_inst.YData = tauB_sc(k,1);
